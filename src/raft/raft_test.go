...
func TestMy(t *testing.T) {
	servers := 3
	cfg := make_config(t, servers, false) // 创建环境 servers 个节点
	defer cfg.cleanup()

	cfg.begin("Test begin")

	cfg.checkOneLeader()	// 检测是否仅有一个leader
	fmt.Printf("Ok only one Leader\n")
	...
	if term1 != term2 {						// 由于未让任何节点kill,不应该更换term
		fmt.Printf("waring : term changed")
	}

	leader1 := cfg.checkOneLeader()

	cfg.disconnect(leader1)				// kill掉当前leader
	cfg.checkOneLeader()				// 检查是否新生成leader
	fmt.Printf("Ok leader is create\n")

	cfg.connect(leader1)				// leader1 重新连接
	leader2 := cfg.checkOneLeader()		// 即使leader1 重连也应该仅有1个leader

	cfg.disconnect(leader2)
	cfg.disconnect((leader2+1)%servers)	// 断开两个
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()					// 一半都断开,不应该有leader被选举出来
	fmt.Printf("Ok no Leader\n")

	cfg.connect((leader2 + 1)%servers)
	cfg.checkOneLeader()				// 目前有2个在线,应该选举出leader
